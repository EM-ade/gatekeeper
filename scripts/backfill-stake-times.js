  ...
  try {
    ...
    for (const posDoc of positionsSnapshot.docs) {
      ...
      if (!txSnapshot.empty) {
        ...
        console.log(`   Found earliest stake transaction: ${stakeStartTime.toDate().toLocaleString()}`);
      } else {
        // Fallback to updated_at or now
        stakeStartTime = position.updated_at || admin.firestore.Timestamp.now();
        console.log(`   No stake transaction found, using updated_at: ${stakeStartTime.toDate().toLocaleString()}`);
      }

      // Update position
      await db.collection("staking_positions").doc(userId).update({
