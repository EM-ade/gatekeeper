...
async function fundPool() {
  ...
  try {
    ...
    if (!poolDoc.exists) {
      console.error(
        "❌ Staking pool not initialized. Run init-staking-pool.js first."
      );
      process.exit(1);
    }
    ...
    console.log(
      `⚠️  IMPORTANT: Before proceeding, ensure you have transferred`
    );
    ...
    console.log(
      `   Do you want to update the Firestore pool with this amount?`
    );
    console.log(
      `   This will enable ${fundAmount.toFixed(
        4
      )} SOL in rewards for stakers.\n`
    );
